...
int get_from_uri(const char *uri_str, unsigned char **data, size_t *length) {
  ...
  curl = curl_easy_init();
  if (curl == NULL) {
	  WLog_ERR(TAG, "get_easy_init() failed");
    return -1;
  }
  ...
  if (rv != 0) {
	  WLog_ERR(TAG, "curl_easy_perform() failed: %s (%d)", curl_error, rv);
    return -1;
  }
  /* copy data */
  *data = curl_data.data;
...
static int parse_ldap_uri(const char *in, LDAPURLDesc ** out)
{
  if (ldap_url_parse(in, out) != 0) {
	  WLog_ERR(TAG, "ldap_url_parse() failed");
    return -1;
  }
...
static int parse_uri(const char *str, uri_t **uri)
{
  ...
  *uri = malloc(sizeof(uri_t));
  ...
  if (strchr(str, ':') == NULL) {
    ...
    if (rv != 0)
    	WLog_ERR(TAG, "parse_generic_uri() failed: %d", rv);
    ...
    if (rv != 0)
    	WLog_ERR(TAG, "parse_ldap_uri() failed: %d", rv);
#else
    rv = -1;
    WLog_ERR(TAG, "Compiled without ldap support");
#endif
  } else {
    (*uri)->scheme = unknown;
    rv = 0;
  ...
  *data = NULL;
  ...
  if (rv == 301 || rv == 302) {
    ...
    if (rv != 0) {
      free(ruri);
      free(buf);
      WLog_ERR(TAG, "parse_uri() failed: %d", rv);
      return -1;
    }
    if (ruri->scheme != http) {
      free(ruri);
...
static int get_ldap(uri_t *uri, unsigned char **data, size_t *length)
{
  ...
  *data = NULL;
  ...
  if (ldap == NULL) {
    ldap_unbind_s(ldap);
    WLog_ERR(TAG, "ldap_init() failed: %s", strerror(errno));
    return -1;
  }
  rv = ldap_simple_bind_s(ldap, NULL, NULL);
  if (rv != LDAP_SUCCESS) {
    ldap_unbind_s(ldap);
    WLog_ERR(TAG, "ldap_simple_bind_s() failed: %s", ldap_err2string(rv));
    return -1;
  }
  /* search an item */
  ...
  if (rv != LDAP_SUCCESS) {
    ldap_unbind_s(ldap);
    WLog_ERR(TAG, "ldap_search_s() failed: %s", ldap_err2string(rv));
    return -1;
  }
  ...
  if (vals == NULL) {
    ...
    WLog_ERR(TAG, "ldap_ldap_get_values_len() failed: %s", ldap_err2string(rv));
    return -1;
  }
...
int get_from_uri(const char *str, unsigned char **data, size_t *length)
{
  ...
  switch (uri->scheme) {
    case file:
      ...
      if (rv != 0)
    	  WLog_ERR(TAG, "get_file() failed: %d", rv);
      break;
    case http:
      rv = get_http(uri, data, length, 0);
      if (rv != 0)
    	  WLog_ERR(TAG, "get_http() failed: %d", rv);
      break;
    case ldap:
#ifdef HAVE_LDAP
      rv = get_ldap(uri, data, length);
      if (rv != 0)
    	  WLog_ERR(TAG, "get_ldap() failed: %d", rv);
#else
      rv = -1;
      WLog_ERR(TAG, "Compiled without LDAP support");
#endif
