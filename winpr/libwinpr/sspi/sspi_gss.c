        ...
        PSecBuffer pPackedContext, HANDLE* pToken)
{
        ...
        if (!(g_GssApi && g_GssApi->ExportSecurityContext))
                return SEC_E_UNSUPPORTED_FUNCTION;

        status = g_GssApi->ExportSecurityContext(phContext, fFlags, pPackedContext, pToken);
        WLog_Print(TAG, WLOG_DEBUG, "ExportSecurityContext: %s (0x%08"PRIX32")",
...
#if 0
#ifdef WITH_GSSAPI
GSSAPI_FUNCTION_TABLE* SEC_ENTRY InitSecurityInterfaceEx(DWORD flags)
{
	InitOnceExecuteOnce(&g_Initialized, sspi_GssApiInit, &flags, NULL);
	WLog_Print(TAG, WLOG_DEBUG, "InitSecurityInterfaceEx");
	return g_GssApiLink;
}
#else
GSSAPI_FUNCTION_TABLE* SEC_ENTRY InitSecurityInterfaceEx(DWORD flags)
