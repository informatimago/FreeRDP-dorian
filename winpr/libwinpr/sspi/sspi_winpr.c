...
#define PRINT(...)							\
	...
	if (identity->CspData)
	{
		...
		PRINT_IDENTITY_ITEM(CspData->CardName);
		PRINT_IDENTITY_ITEM(CspData->ReaderName);
...
void sspi_CredentialsFree(SSPI_CREDENTIALS* credentials)
{
	...
	if (memset_s(credentials->identity.Pin, 48, 0,
	             pinLength))  /* 48 = 2 * ( CredSSP formatted pin length ) */
...
const int Guard =  0xDECEA5ED;
inline ULONG * guard_pointer(void * buffer, ULONG size)
{
	return (ULONG *)((char *)buffer + sizeof(ULONG) * ((size + sizeof(ULONG) - 1)/ sizeof(ULONG)));
}

inline ULONG size_with_guard(ULONG size)
{
	return sizeof (ULONG) * (1 + ((size + sizeof(ULONG) - 1)/ sizeof(ULONG)));
}
...
void* sspi_SecBufferAllocType(PSecBuffer SecBuffer, ULONG size, ULONG BufferType)
{
	...
	/*
	We cannot do that, since they're not all initialized to {0}:
	sspi_SecBufferFree(SecBuffer);
	Keeping a linked list of initialized buffers is not possible
	because they're not allocated dynamically, but are also
	automatic variables,  so they can have the same address as previous ones.
	*/
	SecBuffer->pvBuffer = calloc(1, size_with_guard(size));
	if (!SecBuffer->pvBuffer)
		return NULL;
	ZeroMemory(SecBuffer->pvBuffer, size_with_guard(size));
...
When time for a bigger refactoring is available,
replace the following STRING_*  macros with a struct!
...
inline void string_length_shallow_copy(LPWSTR *  destination, ULONG * destinationLength,
				       LPWSTR source, ULONG sourceLength)
{
	(*destination) = source;
	(*destinationLength) = sourceLength;
}

#define STRING_SHALLOW_COPY(destination, source)				\
	string_length_shallow_copy( & destination, & destination ## Length,	\
		source,  source ## Length)


inline void string_length_set_empty(LPWSTR *  destination, ULONG * destinationLength)
{
	free(*destination);
	(*destination) = 0;
	(*destinationLength) = 0;
...
#define STRING_SET_EMPTY(destination) string_length_set_empty(destination,  destination ## Length)


int string_length_unicode_from_c_string(LPWSTR *  destination, ULONG * destinationLength, const char * source)
{
	string_length_set_empty(destination, destinationLength);
	if (source)
	{
		int status = ConvertToUnicode(CP_UTF8, 0, source, -1, destination, 0);
		if (status <= 0)
		{
			return -1;
		}
		(*destinationLength) = (ULONG)(status - 1);
		(*destination)[(*destinationLength)] = 0;
	}
	return 0;
}

#define STRING_UNICODE_FROM_C(destination, source)			\
	do								\
	{								\
		WLog_DBG(TAG, #source "=%s; strlen(" #source ")=%d",	\
			source, strlen(source));			\
		if(string_length_unicode_from_c_string(&(destination),	\
				&(destination ## Length), source) < 0)	\
		{							\
			return -1;					\
		}							\
	}while (0)
...
int string_length_unicode_copy_from_unicode_string(LPWSTR *  destination, ULONG * destinationLength,
	LPWSTR  source, ULONG sourceLength)
{
	(*destination) = 0;
	(*destinationLength) = 0;
	if (0 < sourceLength)
	{
		LPWSTR copy = calloc(sourceLength + 1, sizeof(WCHAR));
		...
		CopyMemory(copy, source, sourceLength * sizeof(WCHAR));
		copy[sourceLength] = 0;
		(*destination) = copy;
		(*destinationLength) = sourceLength;
	}
...
#define STRING_UNICODE_COPY(destination, source)					\
	do										\
	{										\
		if(string_length_unicode_copy_from_unicode_string(&(destination),	\
				&(destination ## Length),				\
...
#define DECLARE_STRING_UNICODE(name) LPWSTR name; ULONG name ## Length
                         ...
                         const char* password)
{
	...
	STRING_UNICODE_FROM_C(identity->User, user);
	STRING_UNICODE_FROM_C(identity->Domain, domain);
	STRING_UNICODE_FROM_C(identity->Password, password);
	sspi_IdentityPrint(identity);
	return 1;
}


static int setCSPData(SEC_WINNT_AUTH_IDENTITY_CSPDATADETAIL** pIdentityCspData,
	UINT32 keySpec,
	const char* cardName,
	...
	const char* cspName)
{
	*pIdentityCspData = calloc(1, sizeof(SEC_WINNT_AUTH_IDENTITY_CSPDATADETAIL));
	...
	STRING_UNICODE_FROM_C((*pIdentityCspData)->CardName, cardName);
	STRING_UNICODE_FROM_C((*pIdentityCspData)->ReaderName, readerName);
	STRING_UNICODE_FROM_C((*pIdentityCspData)->ContainerName, containerName);
	STRING_UNICODE_FROM_C((*pIdentityCspData)->CspName, cspName);

	return ((*pIdentityCspData)->CardNameLength + (*pIdentityCspData)->ReaderNameLength +
	        (*pIdentityCspData)->ContainerNameLength + (*pIdentityCspData)->CspNameLength);
}
                                   ...
                                   const char* domainHint)
{
	...
	cardName = "IAS-ECC";
	containerName = "Clé d'authentification1";;
	userHint = "BOURGUIGNONPA";
	domainHint = "AC.INT";
	// domainHint = 0;
	WLog_DBG(TAG, "Hard wired: %s %s %s %s %s", readerName, cardName, containerName, userHint,  domainHint);
	...
	{
		if (setCSPData(&identity->CspData,
				keySpec, cardName, readerName, containerName,
				cspName) <= 0) /* setCSPData returns a total length */
			return -1;
	}

	/* /\* begin kludge to test smartcard logon with ntml *\/ */
	/* STRING_UNICODE_FROM_C(identity->User, userHint); */
	/* STRING_UNICODE_FROM_C(identity->Domain, domainHint); */
	/* STRING_UNICODE_FROM_C(identity->Password, "Minint-19!"); */
	/* /\* end kludge *\/ */

	STRING_UNICODE_FROM_C(identity->Pin, pin);
	STRING_UNICODE_FROM_C(identity->UserHint, userHint);
	STRING_UNICODE_FROM_C(identity->DomainHint, domainHint);

	sspi_IdentityPrint(identity);
	return 1;
}

int CopyCSPData(SEC_WINNT_AUTH_IDENTITY* identity, SEC_WINNT_AUTH_IDENTITY* srcIdentity,
                int identityCspDataLength)
{
	if (identity->CspData == NULL && srcIdentity->CspData != NULL && identityCspDataLength)
	{
		...
		STRING_SHALLOW_COPY(identity->CspData->CardName, srcIdentity->CspData->CardName);
		STRING_SHALLOW_COPY(identity->CspData->ReaderName, srcIdentity->CspData->ReaderName);
		STRING_SHALLOW_COPY(identity->CspData->ContainerName, srcIdentity->CspData->ContainerName);
		STRING_SHALLOW_COPY(identity->CspData->CspName, srcIdentity->CspData->CspName);
	}
...
int sspi_CopyAuthIdentity(SEC_WINNT_AUTH_IDENTITY* identity, SEC_WINNT_AUTH_IDENTITY* srcIdentity)
{
	...
	if (srcIdentity->Flags == SEC_WINNT_AUTH_IDENTITY_ANSI)
	{
		...
		{
			status = sspi_SetAuthIdentity_Smartcard(identity, (char*) srcIdentity->Pin,
	...
	if (srcIdentity->Password)
	{
		DECLARE_STRING_UNICODE(tempPassword);

		STRING_SHALLOW_COPY(tempPassword, srcIdentity->Password);
		if (tempPasswordLength > 256)
		{
			tempPasswordLength /= SSPI_CREDENTIALS_HASH_LENGTH_FACTOR; /* TODO: WHAT THE FUCK!?!? */
		}

		STRING_UNICODE_COPY(identity->User, srcIdentity->User);
		STRING_UNICODE_COPY(identity->Domain, srcIdentity->Domain);
		STRING_UNICODE_COPY(identity->Password, tempPassword);
	}
	else if (srcIdentity->CspData)
	{
		int identityCspDataLength = (sizeof(srcIdentity->CspData->KeySpec)
			+ srcIdentity->CspData->CardNameLength
			+ srcIdentity->CspData->ReaderNameLength
			+ srcIdentity->CspData->ContainerNameLength
			+ srcIdentity->CspData->CspNameLength);

		STRING_UNICODE_COPY(identity->Pin, srcIdentity->Pin);
		...
		/* STRING_UNICODE_COPY(identity->User, srcIdentity->User); */
		/* STRING_UNICODE_COPY(identity->Domain, srcIdentity->Domain); */
		/* STRING_UNICODE_COPY(identity->Password, srcIdentity->Password); */
		/* /\* end kludge *\/ */

		STRING_UNICODE_COPY(identity->UserHint, srcIdentity->UserHint);
		STRING_UNICODE_COPY(identity->DomainHint, srcIdentity->DomainHint);
	}
...
const SEC_CHAR* sspi_GetSecurityFunctionTableName(void* table)
{
	UINT32 index;
	UINT32 aPackages = sizeof(SecPkgInfoA_LIST) / sizeof(*(SecPkgInfoA_LIST));
	UINT32 wPackages = sizeof(SecPkgInfoW_LIST) / sizeof(*(SecPkgInfoW_LIST));

	for (index = 0; index < aPackages; index++)
	{
		if (table ==  SecurityFunctionTableA_NAME_LIST[index].SecurityFunctionTable)
		{
			return SecurityFunctionTableA_NAME_LIST[index].Name;
		}
	}

	for (index = 0; index < wPackages; index++)
	{
		if (table == SecurityFunctionTableW_NAME_LIST[index].SecurityFunctionTable)
		{
			LPSTR buffer;
			if(ConvertFromUnicode(CP_UTF8, 0, SecurityFunctionTableW_NAME_LIST[index].Name, -1,
					(LPSTR*)&buffer, 0, NULL, FALSE) >= 1)
			{
				UINT32 index;
				for (index = 0; index < aPackages; index++)
				{
					if (strcasecmp(buffer, SecurityFunctionTableA_NAME_LIST[index].Name) == 0) /* TODO: there's a case discrepancy between internal and external names! cf. credssp.c:31 vs. sspi_winpr.c:124 */
					{
						free(buffer);
						return (SEC_CHAR *)SecurityFunctionTableA_NAME_LIST[index].Name;
					}
				}
			}
			static SEC_CHAR result[80];
...
{
	UINT32 index;
	UINT32 cPackages = sizeof(SecPkgInfoA_LIST) / sizeof(*(SecPkgInfoA_LIST));
...
void warnSecurityStatusError(SECURITY_STATUS status,
	void* table,
	SEC_CHAR* Function)
{
	if (IsSecurityStatusError(status))
	{
		WLog_WARN(TAG, "%s->%s status %s [0x%08"PRIX32"]",
			sspi_GetSecurityFunctionTableName(table),
			Function,
...
void traceSecurityStatusError(SECURITY_STATUS status,
	void* table,
	SEC_CHAR* Function)
{
	WLog_DBG(TAG, "%s->%s status %s [0x%08"PRIX32"]",
		sspi_GetSecurityFunctionTableName(table),
		Function,
		GetSecurityStatusString(status),
		status);
        ...
        void* pvGetKeyArgument, PCredHandle phCredential, PTimeStamp ptsExpiry)
{
	...
	warnSecurityStatusError(status, table, "AcquireCredentialsHandleW");
	return status;
}
        ...
        void* pvGetKeyArgument, PCredHandle phCredential, PTimeStamp ptsExpiry)
{
	...
	status = table->AcquireCredentialsHandleA(pszPrincipal, pszPackage, fCredentialUse,
	         pvLogonID, pAuthData, pGetKeyFn, pvGetKeyArgument, phCredential, ptsExpiry);
	warnSecurityStatusError(status, table, "AcquireCredentialsHandleA");
	return status;
}
        ...
        PSecBuffer pPackedContext, HANDLE* pToken)
{
	...
	warnSecurityStatusError(status, table, "ExportSecurityContext");
	return status;
}
...
SECURITY_STATUS SEC_ENTRY winpr_FreeCredentialsHandle(PCredHandle phCredential)
{
	...
	warnSecurityStatusError(status, table, "FreeCredentialsHandle");
	return status;
}
        ...
        PSecBuffer pPackedContext, HANDLE pToken, PCtxtHandle phContext)
{
	...
	warnSecurityStatusError(status, table, "ImportSecurityContextW");
	return status;
}
        ...
        PSecBuffer pPackedContext, HANDLE pToken, PCtxtHandle phContext)
{
	...
	warnSecurityStatusError(status, table, "ImportSecurityContextA");
	return status;
}
        ...
        ULONG ulAttribute, void* pBuffer)
{
	...
	warnSecurityStatusError(status, table, "QueryCredentialsAttributesW");
	return status;
}
        ...
        ULONG ulAttribute, void* pBuffer)
{
	...
	warnSecurityStatusError(status, table, "QueryCredentialsAttributesA");
	return status;
}
        ...
        PSecBufferDesc pOutput, PULONG pfContextAttr, PTimeStamp ptsTimeStamp)
{
	...
	warnSecurityStatusError(status, table, "AcceptSecurityContext");
	return status;
}
...
SECURITY_STATUS SEC_ENTRY winpr_ApplyControlToken(PCtxtHandle phContext, PSecBufferDesc pInput)
{
	...
	warnSecurityStatusError(status, table, "ApplyControlToken");
	return status;
}
...
SECURITY_STATUS SEC_ENTRY winpr_CompleteAuthToken(PCtxtHandle phContext, PSecBufferDesc pToken)
{
	...
	warnSecurityStatusError(status, table, "CompleteAuthToken");
	return status;
}
...
SECURITY_STATUS SEC_ENTRY winpr_DeleteSecurityContext(PCtxtHandle phContext)
{
	...
	warnSecurityStatusError(status, table, "DeleteSecurityContext");
	return status;
}
...
SECURITY_STATUS SEC_ENTRY winpr_ImpersonateSecurityContext(PCtxtHandle phContext)
{
	...
	warnSecurityStatusError(status, table, "ImpersonateSecurityContext");
	return status;
}
        ...
        PSecBufferDesc pOutput, PULONG pfContextAttr, PTimeStamp ptsExpiry)
{
	...
	warnSecurityStatusError(status, table, "InitializeSecurityContextW");
	return status;
}
        ...
        PSecBufferDesc pOutput, PULONG pfContextAttr, PTimeStamp ptsExpiry)
{
	...
	status = table->InitializeSecurityContextA(phCredential, phContext,
	         pszTargetName, fContextReq, Reserved1, TargetDataRep,
	         pInput, Reserved2, phNewContext, pOutput, pfContextAttr, ptsExpiry);
	warnSecurityStatusError(status, table, "InitializeSecurityContextA");
	return status;
}
        ...
        void* pBuffer)
{
	...
	warnSecurityStatusError(status, table, "QueryContextAttributesW");
	return status;
}
        ...
        void* pBuffer)
{
	...
	warnSecurityStatusError(status, table, "QueryContextAttributesA");
	return status;
}
...
SECURITY_STATUS SEC_ENTRY winpr_QuerySecurityContextToken(PCtxtHandle phContext, HANDLE* phToken)
{
	...
	warnSecurityStatusError(status, table, "QuerySecurityContextToken");
	return status;
}
        ...
        void* pBuffer, ULONG cbBuffer)
{
	...
	warnSecurityStatusError(status, table, "SetContextAttributesW");
	return status;
}
        ...
        void* pBuffer, ULONG cbBuffer)
{
	...
	warnSecurityStatusError(status, table, "SetContextAttributesA");
	return status;
}
...
SECURITY_STATUS SEC_ENTRY winpr_RevertSecurityContext(PCtxtHandle phContext)
{
	...
	warnSecurityStatusError(status, table, "RevertSecurityContext");
	return status;
}
        ...
        ULONG MessageSeqNo, PULONG pfQOP)
{
	...
	warnSecurityStatusError(status, table, "DecryptMessage");
	return status;
}
        ...
        PSecBufferDesc pMessage, ULONG MessageSeqNo)
{
	...
	warnSecurityStatusError(status, table, "EncryptMessage");
	return status;
}
        ...
        PSecBufferDesc pMessage, ULONG MessageSeqNo)
{
	...
	warnSecurityStatusError(status, table, "MakeSignature");
	return status;
}
        ...
        ULONG MessageSeqNo, PULONG pfQOP)
{
	...
	warnSecurityStatusError(status, table, "VerifySignature");
	return status;
}
